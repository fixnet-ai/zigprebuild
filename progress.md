...
| 版本 | 日期 | 内容 |
|------|------|------|
| v0.35.0 | 2026-09-19 | NNG v1.12.3 出库 + nng_c 绑定模块（10 target 静态库；钉 stable tag，`main` 是 2.0-beta 禁用；musl 下 `-DNNG_HAVE_ARC4RANDOM=OFF` 走 getrandom 分支）；commit `c18cb93`（出库 + nng_c curated re-export）+ `5f25e4e`（nng_c 补 PUB0/SUB0 opens、`nng_ctx_set_ms` 超时、Windows SD 指针选项） |
